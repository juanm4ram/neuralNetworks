...
(function () {
  ...
  var EN = {
    ...
    "intro_p04a": "Which fragments are worth representing as separate units and labelling with a unique ID depends on how frequently they appear in the texts used to train the tokenizer.",
    "intro_p04b": "For example, a tokenizer will not assign an ID to a character fragment such as &quot;jkdjfkdjfk&quot; because it does not occur in any word. In contrast, &quot;in&quot; is a common fragment in English words such as &quot;intelligence&quot;, &quot;inside&quot;, and &quot;intent&quot;.",
    ...
    "intro_p10": "The function's input will be represented by the <span class=\"concept concept--text\">numerical ID of each token</span>, in the same order. For example, using illustrative values:",
    "intro_p11": "Each component of the vector corresponds to a token in the text. This example has <span class=\"concept concept--gold\">eight</span>. The actual numbers depend on each tokenizer's vocabulary; longer texts will produce vectors with more components.",
    ...
    "intro_p20": "Explaining every stage in depth can take quite a while. Here we will begin with the first one used—the stage furthest to the right—<em>embeddings</em>, and then explain how Transformer blocks add context to them.",
    "intro_p21": "Once the tokenizer converts text into IDs, each ID is used to look up a vector in a large embedding table.<br><span class=\"concept concept--vector\">An embedding</span> is a <span class=\"concept concept--vector\">vector of numbers</span> learned during training and assigned to a token. Unlike the ID defined by the tokenizer, the embedding's coordinates are adjusted according to the contexts in which the token appears and eventually capture relationships in meaning.",
    ...
    "intro_p26": "Once the row is found, the complete vector replaces the ID as the token's representation for the next stages of the model function. Transformations can now be applied to the embedding matrix, which is the objective of the next stage.<br>That is:",
    ...
    "intro_p43": "In simple terms, Transformer blocks help the model <span class=\"concept concept--context\">pay attention to the key words</span> in the text and use that information to <span class=\"concept concept--context\">put each word into context</span>. A word is no longer interpreted in isolation; it is represented according to the words around it.",
    "intro_p44": "Let's look at another example:",
    ...
    "intro_probability_title": "The probability function",
    "intro_embeddings_title": "Embeddings",
    ...
    t031: 'In other words, performance improves predictably but with <em>diminishing returns</em>: to lower the loss a lot, you need to increase compute enormously. We noted down some approximate points from Figure 1 of the cited paper, which measure test loss as compute increases:',
    t031a: '(X axis = training compute, Y axis = test loss, both on logarithmic scales):',
  ...
  var ES_EXTRA = {
    ...
    chartXLabel: 'cómputo de entrenamiento',
    chartError: 'Error (MSE) = ',
    layerPrev: 'CAPA N−1',
    layerCurr: 'CAPA N',
    layerNext: 'CAPA N+1',
